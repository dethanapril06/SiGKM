...
class DashboardController extends Controller
{
    ...
    public function anggotaGkm(): View
    {
        return view('dashboard.anggota-gkm', [
            ...
            'statusPekerjaan' => [
                'ringkasan_draft' => RingkasanPerkuliahan::where('status', 'draft')->count(),
                'ringkasan_ditolak' => RingkasanPerkuliahan::where('status', 'ditolak')->count(),
                'temuan_draft' => Temuan::where('status', 'draft')->count(),
                'notulen_draft' => NotulenRtm::where('status', 'draft')->count(),
                'notulen_ditolak' => NotulenRtm::where('status', 'ditolak')->count(),
            ],
            'ringkasanTerbaru' => RingkasanPerkuliahan::with(['perkuliahan.mataKuliah', 'perkuliahan.kelas'])->latest()->limit(5)->get(),
            'temuanTerbaru' => Temuan::with('evaluasiIndikator.semester.tahunAkademik')->latest()->limit(5)->get(),
            'notulenTerbaru' => NotulenRtm::with('jadwalRtm.semester.tahunAkademik')->latest()->limit(5)->get(),
