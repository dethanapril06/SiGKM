...
@section('content')
    <div class="d-flex justify-content-between align-items-center py-3 mb-4">
        ...
        <a href="{{ ($selectedScope ?? '') === 'prodi' ? route('temuan-evaluasi.prodi') : route('temuan-evaluasi.fakultas') }}" class="btn btn-secondary">
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('temuan-evaluasi.store') }}" method="POST">
                @csrf
                <input type="hidden" name="evaluasi_indikator_id" id="evaluasi_indikator_id" value="{{ old('evaluasi_indikator_id') }}">

                {{-- Scope Switcher: Fakultas vs Prodi --}}
                <div class="mb-4">
                    <label class="form-label fw-bold">Lingkup Temuan</label>
                    <div class="btn-group w-100" role="group">
                        <input type="radio" class="btn-check" name="scope_type" id="scope_fakultas" value="fakultas" @checked(($selectedScope ?? 'fakultas') === 'fakultas')>
                        <label class="btn btn-outline-primary" for="scope_fakultas">
                            <i class="bx bx-building me-1"></i> Fakultas (Standar Mutu & Indikator Mutu)
                        </label>

                        <input type="radio" class="btn-check" name="scope_type" id="scope_prodi" value="prodi" @checked(($selectedScope ?? '') === 'prodi')>
                        ...
                        <div id="section_fakultas">
                            <div class="mb-3">
                                ...
                                <select id="fakultas_evaluasi_id" class="form-select @error('evaluasi_indikator_id') is-invalid @enderror">
                                    ...
                                    @foreach ($evaluasiFakultas as $ev)
                                        @php
                                            ...
                                            $isSelected = (string) old('evaluasi_indikator_id') === (string) $ev->id;
                                        @endphp
                                        <option value="{{ $ev->id }}"
                                            @selected($isSelected)
                                ...
                                <div class="form-text text-muted">
                                    <i class="bx bx-info-circle me-1"></i> Menampilkan evaluasi indikator mutu dengan capaian <strong>Hampir Tercapai</strong> atau <strong>Belum Tercapai</strong>.
                                </div>
                            </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="tingkat_risiko_id"
                        ...
                        @foreach ($tingkatRisiko as $risiko)
                            <option value="{{ $risiko->id }}"
                                {{ old('tingkat_risiko_id') == $risiko->id ? 'selected' : '' }}>
                                {{ $risiko->nama_tingkat }} (Nilai: {{ $risiko->nilai }})
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="deskripsi_risiko" rows="3"
                        class="form-control @error('deskripsi_risiko') is-invalid @enderror"
                        placeholder="Jelaskan risiko jika temuan ini tidak ditindaklanjuti">{{ old('deskripsi_risiko') }}</textarea>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="dampak_risiko" rows="3"
                        class="form-control @error('dampak_risiko') is-invalid @enderror"
                        placeholder="Jelaskan potensi dampak yang timbul">{{ old('dampak_risiko') }}</textarea>
                    @error('dampak_risiko')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function updateFakultasDetail() {
                ...
                if (fakultasEvaluasiSelect.value && selectedOpt) {
